...
// HTML를 모두 읽었을때, 해당 함수가 실행
document.addEventListener('DOMContentLoaded' , function(){
    // 2. h1 요소를 객체로 1개(첫번째)* 가져오기
    const header = document.querySelector('h1');
        // document.querySelector('h1');                해당 요소를 객체로 호출
        // document.querySelector('h1').value;          해당 요소의 입력된 값 호출
            // [* 단 value 입력관련 요수만 가능] : input, select, textarea 등 / div,h1,span [X]
        // document.querySelector('h1').innerHTML;      해당 요소 사이의 HTML를 텍스트 호출 [마크업 코드 읽어옴]


    header.textContent = "HEADERS";                 // DOM객체명.textConent = '값; : 텍스트 호출 [HTML 렌더링 결과물]
    ...
    b.innerHTML = `<h1>textContent 속성</h1>`;      // html태그는 태그대로
});